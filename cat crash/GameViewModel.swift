...
final class GameViewModel: ObservableObject {
    ...
    enum Constant {
        static let lebarpapan = 8
        static let tinggipapan = 8
        static let rows = (0..<tinggipapan)
        static let columns = (0..<lebarpapan)
        static var hitungcell: Int { lebarpapan * tinggipapan }
        ...
        static let cellContents = ["blackcat", "chinesecat", "whitecat", "crycat", "nyancat","pusheen"]
    ...
    init() {
        cells = Self.papanbaru()
    }

    func tukar(_ sourceIndex: Int, with destinationIndex: Int) {
    ...
    func buangsama() {
        cells = Self.buangsama(cells, removing: Self.matches(for: cells))
    }
    ...
    static func bersebelahan(_ sourceIndex: Int, to destinationIndex: Int) -> Bool {
    ...
    private static func index(x: Int, y: Int) -> Int? {
        Constant.columns.contains(x) && Constant.rows.contains(y)
            ? x + y * Constant.lebarpapan
            : nil
    }

    private static func coordinate(for index: Int) -> (Int, Int) {
        (index % Constant.lebarpapan, index / Constant.lebarpapan)
    }

    private static func papanbaru() -> [Cell] {
        var board: [Cell]
        repeat {
            board = .init(
                (0..<Constant.hitungcell).map { index in
                    Cell(position: index)
                }
            )
    ...
    private static func buangsama(_ board: [Cell], removing matches: Set<Int>) -> [Cell] {
        with(board) { board in
    ...
    private static func collapsed(board: [Cell]) -> [Cell] {
        Constant.columns.map { x -> [Cell] in
            ...
            return [Cell(position: indices[0])]
                + indices.prefix(upTo: indexOfMatch).map { with(board[$0]) { $0.position += Constant.lebarpapan } }
